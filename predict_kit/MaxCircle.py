...
class MaxCircle():
    @staticmethod
    def max_circle(original_img, calibrated_img):
        """
        获取裂缝区域最大内切圆

        Args:
            original_img: 待处理的原图像
            calibrated_img: 分割算法获取的二值图像
        """
        
        # 灰度处理
        original_img = np.array(original_img)
        original_img = cv2.cvtColor(original_img, cv2.COLOR_BGR2RGB)
        # img_gray = cv2.cvtColor(calibrated_img, cv2.COLOR_BGR2GRAY)
        img_gray = cv2.bitwise_not(calibrated_img)
        ...
        for c in contours:
            ...
            for id in rand_index:
                tr = MaxCircle.iterated_optimal_incircle_radius_get(c, in_point[id][0], in_point[id][1], radius, big_r, precision)
            ...
            for expansion_circle in expansion_circle_list:
                ...
                if radius_s > 0:
                    if radius_s == max_radius: 
                        # 最大内切圆，用蓝色标注 
                        cv2.circle(original_img, (int(max_center[0]), int(max_center[1])), int(max_radius), (255, 0, 0), 2)
                    else:
                        # 其他内切圆，用青色标注
                        cv2.circle(original_img, (int(center_s[0]), int(center_s[1])), int(radius_s), (255, 245, 0), 2)
                    formatted_width = f"{radius_s * 2:.2f}"
                    cv2.putText(original_img, f"{formatted_width}", (int(center_s[0]-10), int(center_s[1]) - 10),
