...
class Unet(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"    : 'pth/resnet50_unet_best_epoch_weights.pth',
        #--------------------------------#
        #   所需要区分的类的个数+1
        #--------------------------------#
        "num_classes"   : 2,
    ...
    def detect_image(self, image, count=False, name_classes=None):
        ...
        with torch.no_grad():
            ...
            pr_flat = pr.flatten()
            # 获取唯一的像素种类和它们的出现次数
            unique_classes, class_counts = np.unique(pr_flat, return_counts=True)
            # 唯一像素种类的个数
            ...
            # 设置一个阈值，不要很小的部分
            if num_classes > 1 and class_counts[1] < 500:
                Tag = False
        ...
        if self.mix_type == 0:
            ...
            if not Tag:
                ...
                if yinying:
                    ...
                    transparent_black_layer = Image.new("RGB", image.size, (0, 0, 0))

                    # 调整混合系数，使 image 蒙上透明黑色图层
                    blend_factor = 0.35  # 可调整蒙上透明黑色的程度，值越大蒙得越重
                    image = Image.blend(image, transparent_black_layer, blend_factor)
